...
import SourcesHP from './SourcesReports';
import { GrResources } from 'react-icons/gr';
import { Result } from '@yext/search-headless-react';
import { FaWandMagicSparkles } from 'react-icons/fa6';
import { usePageContext } from '../../utils/usePageContext';
import FollowUpButton from '../FollowUpButton';
import SourcesReports from './SourcesReports';
...
export default function GenerativeAnswerWrapperReports({
  answer,
  results,
}: Props) {
  ...
  return answerCitationSplit ? (
    <div className="w-full flex flex-col gap-6 max-w-3xl text-[#0a3366]">
      ...
      <div className="flex flex-col gap-4">
        ...
        <SourcesReports sources={sourcesArray} />
      </div>
      <FollowUpButton />
    </div>
  ) : (
